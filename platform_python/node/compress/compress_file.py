...
class CompressFile(NodeBase):
    ...
    def check_params(self):
        ...
        if not CResult.result_sucess(result):
            return result
    ...
    def check_input(self):
        ...
        if not CResult.result_sucess(result):
            return result
        if not self.input_list or self.input_list is None:
            return CResult.merge_result(
                self.NODE_EXCEPTION,
    ...
    def compress(self, file_name_with_path) -> CResult:
        try:
            ...
            file_name_with_suffix = CFile.change_suffix(file_main_name, self.target_suffix)
            target_path_with_name = CFile.path_join(self.target_path, file_name_with_suffix)
    ...
    def run(self):
        ...
        if not CResult.result_sucess(result):
            return result
        result_update = CResult()
        for file_name_with_path in self.input_list:
            compress_result = self.compress(file_name_with_path)
